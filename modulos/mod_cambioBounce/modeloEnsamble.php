<?
	...
	class modeloEnsamble{
		...
		public function guardarReemplazo($imei,$imeiB,$serialB){
			$sql="SELECT id_radio FROM equipos WHERE imei='".$imei."'";
			$res=mysql_query($sql,$this->conectarBd());
			$row=mysql_fetch_array($res);
			$sql="INSERT INTO reemplazosBounce (id_radio,imei,serial) VALUES ('".$row["id_radio"]."','".$imeiB."','".$serialB."')";
			$res=mysql_query($sql,$this->conectarBd());
			if($res){
				$sql="UPDATE equipos set status='BOUNCE',statusProceso='BOUNCE' WHERE id_radio='".$row["id_radio"]."'";
				$res=mysql_query($sql,$this->conectarBd());
				if($res){
					echo "Actualizacion y Reemplazo Guardado";	
				}else{
...
?>
			...
			<br><table border="0" cellpadding="1" cellspacing="1" width="800" style="font-size: 12px;margin: 10px;">
				<tr>
					<td colspan="2">Reemplazo de Equipos para Bounce</td>
					<td>&nbsp;</td>
