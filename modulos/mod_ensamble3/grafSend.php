<?php
    ...
    $sqlTotalEquipos="SELECT count( DISTINCT detalle_ing.id_radio ) AS total,equipos_enviados.lineaEnsamble AS linea FROM detalle_ing INNER JOIN equipos_enviados ON detalle_ing.id_radio = equipos_enviados.id_radio WHERE (f_registro between '".$fechaIni."' AND '".$fechaFin."')AND(id_proc =4) AND equipos_enviados.status IN ('WIP','Validando','ENVIADO') GROUP BY equipos_enviados.lineaEnsamble";
    $resTotalEquipos=mysql_query($sqlTotalEquipos,conexionBd());
    $valores="";    $nombres="";
    while($rowEquipos=mysql_fetch_array($resTotalEquipos)){
        if($valores==""){
            $valores=$valores.$rowEquipos["total"];
        }else{
            $valores=$valores.",".$rowEquipos["total"];
        }
        if($nombres==""){
            $nombres=$nombres."Linea ".$rowEquipos["linea"];
        }else{
            $nombres=$nombres.",Linea ".$rowEquipos["linea"];
        }
    }
    ...
    $myPicture->drawText(10,20,"Equipos Enviados",array("R"=>255,"G"=>255,"B"=>255));//10,13
