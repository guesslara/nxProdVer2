...
function procesaFormulario(){	
	...
	if(equipos==""){
		...
		//se recupera el nuevo filtro
		filtro=document.getElementById("cboFiltroRecibo").value;
		if(filtro=="Selecciona"){
			alert("Error: Seleccione un filtro para efectuar el Recibo.");
		}else{
			div="div_grid_ensamble";
			url="controladorEnsamble.php";
			parametros="action=actualizaDatosAlmacen&equipos="+equipos+"&proceso="+proceso+"&id_usuarioEnsamble="+id_usuarioEnsamble+"&filtro="+filtro;
			//alert(parametros);
			metodo="POST";
	...
	$("#txtImeiEnsamble").attr("disabled","true");
	$("#btnNuevaCaptura").removeAttr("disabled");
}
...
function nuevaCaptura(){
	$("#btnNuevaCaptura").attr("disabled","true")
