...
//		set_time_limit(0);
		...
		$resMov=mysql_query($sqlMov,$link);
		if($resMov==false){
		...
		foreach($m_renglones as $renglonX){
			...
			$sql="INSERT INTO equipos (id_personal,id_modelo,imei,serial,lote,mfgdate,statusProceso,f_recibo,h_recibo,num_movimiento) VALUES ('".$_POST['recibe']."','".$_POST['modelo']."','".$m_celdas[0]."','".$m_celdas[1]."','".$_POST["lote"]."','".$_POST['mfgdate']."','Recibo','".date("Y-m-d")."','".date("H:i:s")."','".$mov."'); ";
...
function dame_encabezado_tabla2(){
	...
	mi_html+='<tr>';
		...
		mi_html+='<th>Recibe</th>';
		mi_html+='<th>Proceso</th>';
		mi_html+='<th>Folio</th>';
		mi_html+='<th>MFGDATE</th>';
		mi_html+='</tr>';
	return mi_html;
}
...
function vista_previa(){
	...
	var recibe=$("#txt_recibe").val();
	var proceso=$("#txt_proceso").val();
	var lote=$("#txt_lote").val();
	var mfgdate=$("#txt_mfgdate").val();
	
	if(datos_txt==''||datos_txt==' '||datos_txt==undefined||datos_txt==null){
		alert("Pegue los datos en el area de captura.");
		return;
	}
	if(lote==''||lote==' '||lote==undefined||lote==null||modelo==""||recibe==""||mfgdate==""){
		alert("Capture los datos de entrada.");
		return;
	}
	...
	for(var i=0;i<valores_renglones.length;i++){
		...
		tr_mi_renglon='<tr class="tr_hover">';
		for(var i2=0;i2<valores_renglon_celdas_matriz.length;i2++){
			tr_mi_renglon+='<td>'+valores_renglon_celdas_matriz[i2]+'</td>';
		}
		tr_mi_renglon+='<td>'+modelo+'</td><td>'+recibe+'</td><td>'+proceso+'</td><td>'+lote+'</td><td>'+mfgdate+'</td>';
		tr_mi_renglon+='</tr>';
...
//	var datos_x="ac=subir_datos&lote="+lote+"&datos="+datos_txt;
	var datos_x="ac=subir_datos&lote="+lote+"&modelo="+modelo+"&recibe="+recibe+"&proceso="+proceso+"&mfgdate="+mfgdate+"&datos="+datos_txt;
	//alert(datos_x);
...
<body>
<div id="div_main">
	<div id="div_encabezado" style="background:#000; color:#FFF; height:20px; padding:7px; font-size:12px;">Subir Equipos nuevos a Base de Datos!</div>
...
<?
				$link1=conectarBd();
				$sqlMod="SELECT * FROM cat_modradio";
				$resMod=mysql_query($sqlMod,$link1);
...
?>
            ...
            <tr>
            	<td>Recibe</td>
                <td><input type="text" id="txt_recibe" value="<?=$_SESSION['id_usuario_nx'];?>" /><?=$_SESSION['nombre_nx']." ".$_SESSION['apellido_nx'];?></td>
            </tr>            
            <tr>
            	<td>Proceso</td>
                <td><input type="text" id="txt_proceso" value="Refurbish" /></td>
            </tr>
            <tr>
            	<td>Folio</td>
                <td><input type="text" id="txt_lote" /></td>
            </tr>
            <tr>
            	<td>MFGDATE</td>
                <td><input type="text" id="txt_mfgdate" /></td>
            </tr>
        </table>
    <br /><br />
