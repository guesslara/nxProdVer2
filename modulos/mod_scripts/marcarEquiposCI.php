...
<?
		echo "Cantidad de elementos a MARCAR: ".count($m_data)."<br>";
		for($i=0;$i<count($m_data);$i++){
			//echo $m_data[$i]."<br>";			
			$ci="ci-".date("Y-m-d");
			$sql="UPDATE equipos set cambioIdentidad='".$ci."' WHERE imei = '".$m_data[$i]."'";
			/*********************/
			$res=mysql_query($sql,$link);
			if($res){
				echo "<br>Equipo Marcado Correctamente.";				
			}else{
				echo "<br>Error al marcar la informaci&oacute;n del Equipo";				
			}
			/*********************/
		}			
	}
...
<title>Marcar Equipos como CI.</title>
<style type="text/css">
...
#div_main{ position:absolute; width:100%; height:100%; margin:0px 0px 0px 0px; }
	#div_titulo{ position:relative; height:20px; text-align:center; font-size:large; background-color:#000;color: #FFF;  }
	#div_menu{ position:relative; height:20px; text-align:center; background-color:#000;color: #FFF;  }
	#div_a{ position:relative; height:auto; width:200px; text-align:center; float:left; background-color:#000;color: #FFF; overflow:auto; }
...
th{ /**/ border-right:#ccc 1px solid; border-bottom:#ccc 1px solid; background-color:#000; font-weight:bold; text-align:center; }
...
function buscar_valores_x(){
	if(confirm("Realmente desea Marcar estos Equipos en la Base de Datos")){
...
<body>
<div id="div_main">
	<div id="div_titulo">Registro de Equipos como Cambio Identidad</div>
	<div id="div_menu">
		<a href="#" id="lin_02" onClick="buscar_valores_x()" style="color: #FFF;">Marcar</a> <!--| <a href="#" title="Exportar a Excel">Exportar a Excel</a>-->
	</div>
	<div id="div_a">
		<div>Imei(s) a Marcar:</div>
